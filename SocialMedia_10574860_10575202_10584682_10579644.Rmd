...
token <- "EAACEdEose0cBAPpZCDCtuyjZBSrP019MkCyyVMjXlSePUXCxOCKS2PMlOnd8ZB3f1JMARrH6i1hSwmZAAGpkbyy1KR8mYn73XuqfZBcZAZCuIbmxs41m42yJOosnaPrdLzqMVxTN62UwMwjpMW4Asl1xGlU9BXrw8X0pW324qPXiL9hYYc9iv8vnqfnZAtAiVLFNImjEu9TFZBQZDZD"
```
...
## t-Test Comparing Number of Comments with Likes

When we compare the number of comments to the number of likes using a t-test - the following p-value can be gained.
```{r include=FALSE}

comments_likes_df = data.frame(combined$month, combined$`Average likes`, df$abc) 

names(comments_likes_df)[names(comments_likes_df) == 'combined..Average.likes.'] <- 'Likes'
names(comments_likes_df)[names(comments_likes_df) == 'df.abc'] <- 'Comments'
names(comments_likes_df)[names(comments_likes_df) == 'combined.month'] <- 'Month'

```

```{r echo=FALSE}
cor(comments_likes_df$Likes, comments_likes_df$Comments)
m <- lm(Likes ~ Comments, data = comments_likes_df)
summary(m)$coefficient[2,4]
```

Because the p value is greater than 0.05 we can conclude that there is no significant connection between the number of comments made in a month and the number of likes recieved in a month.
